    ...
    <div className='sticky top-24'>
      ...
      <div className='rounded-[22px] border border-zinc-200 bg-white shadow-sm p-6'>
        ...
        <div className='space-y-6'>
          ...
          {/* Price */}
          <CollapseItem renderProp={<RangeInput pathname={path.battery} queryConfig={queryConfig} />}>
          ...
          {/* Tình trạng pin*/}
          <CollapseItem
            renderProp={BATTERY_HEALTH_OPTIONS.map((item) => {
              return (
                <FilterOptionLink
                  key={item.label}
                  queryConfig={queryConfig}
          ...
          >
            <div className='text-lg font-semibold'>
              <div className='text-lg font-semibold'>Tình trạng pin</div>
            </div>
          </CollapseItem>
          <div className='h-px bg-zinc-100' />
          {/* Điện áp pin*/}
          <CollapseItem
            renderProp={VOLTAGE_OPTIONS.map((item) => {
              return (
                <FilterOptionLink
                  key={item.label}
                  queryConfig={queryConfig}
          ...
          >
            <div className='text-lg font-semibold'>
              <div className='text-lg font-semibold'>Điện áp</div>
            </div>
          </CollapseItem>
          <div className='h-px bg-zinc-100' />
          {/* Điện dung pin*/}
          <CollapseItem
            renderProp={CAPACITY_OPTIONS.map((item) => {
              return (
                <FilterOptionLink
                  key={item.label}
                  queryConfig={queryConfig}
          ...
          >
            <div className='text-lg font-semibold'>
              <div className='text-lg font-semibold'>Điện dung</div>
            </div>
          </CollapseItem>
        </div>
      </div>
