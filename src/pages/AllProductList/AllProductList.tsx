...
export default function AllProductList() {
  const { queryConfig, categoriesData, postsData } = useListQueries({ categoryName: CategoryName.all })
      ...
      {pageData && categories && (
        <div className='pb-8 md:pb-10'>
          ...
          <div className='hidden lg:grid lg:grid-cols-[auto_minmax(0,1fr)] gap-8'>
            {/* Sidebar (desktop) - luôn mounted, width animate 0 <-> SIDEBAR_W */}
            <motion.aside
              aria-hidden={!isFilterOpen}
              animate={{ width: isFilterOpen ? 352 : 0 }}
              initial={false}
              transition={{ duration: isFilterOpen ? 0.34 : 0.28, ease: [0.22, 0.61, 0.36, 1] }}
              className='w-[22rem]' // ❌ bỏ overflow-hidden ở đây để không chặn sticky
            >
